...
def metrics_evaluate(itrain_X, itest_X, itrain_y, itest_y, otrain_X, met, prot, com, cluster_method):
    result = []           
    ood_evaluator = OodEvaluator(itrain_X, itest_X, itrain_y, itest_y, prot, com, cluster_method)
    ood_evaluator(otrain_X, met).get_scores()
    ...
    print("\n")
    result.append({"metric": met,
                    "pca components": com,
                    "clusters": prot,
                    "auroc": ood_evaluator.auroc,
                    "tnr@tpr95": ood_evaluator.tnr_at_tpr95, 
                    "cluster_method":cluster_method,
                    "gmm results": ood_evaluator.gmm_results
                    })
...
if __name__ == "__main__":
    encoder = "key"
    cache_root = f"/data/temiloluwa.adeoti/fourth_experiments/CIFAR10_clus_768_neg_256/exp_1/checkpoint_199_results/ood_results/metrics_result/"
    cache_path_fc = os.path.join(cache_root, f"contrastive_enc_{encoder}_ckpt_199_fc/cache")
    cache_path_ap = os.path.join(cache_root, f"contrastive_enc_{encoder}_ckpt_199_avg_pool/cache")
    iid = "CIFAR10"
    ood = "LSUNResize"

    id_train_fc, id_test_fc, id_train_targ_fc, id_test_targ_fc = load_model_features(cache_path_fc, iid, "fc")
    id_train_ap, id_test_ap, id_train_targ_ap, id_test_targ_ap = load_model_features(cache_path_ap, iid, "avg_pool")

    od_train_fc, od_test_fc, od_train_targ_fc, od_test_targ_fc = load_model_features(cache_path_fc, ood, "fc")
    od_train_ap, od_test_ap, od_train_targ_ap, od_test_targ_ap = load_model_features(cache_path_ap, ood, "avg_pool")


    id_fc_prototypes = load_features("../cache", "new_id_fc_prototypes.npy")
    id_fc_im2cluster = load_features("../cache", "new_id_fc_im2cluster.npy") 
    id_ap_im2cluster = load_features("../cache", "new_id_ap_im2cluster.npy") 
    id_ap_prototypes = load_features("../cache", "new_id_ap_prototypes.npy") 


    oe = OodEvaluator(id_train_ap, id_test_ap, id_train_targ_ap, id_test_targ_ap,
                    num_clusters = 30,
                    pca_com = 10,
                    cluster_method = "kmeans",
                    means = id_ap_prototypes,
                    im2cluster = id_ap_im2cluster,
                    clip = 0.5,
                    clip_metric = "cosine")

    oe(od_test_ap, "cosine")
    oe.get_scores()
    oe.get_auroc()
    print("")
